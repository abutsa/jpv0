...
class User extends MY_Controller {
    ...
    public function user_view() {
        ...
        if ($view == "all")
            ...
            $data['users'] = $this->User_m->get_user_by_level('klien');
        else if ($view == "penulis")
            $data['users'] = $this->User_m->get_user_by_level('penulis');

        $this->open_admin_page('admin/user_view_all', $data);
    }

    //admin, menambah user baru ke database
    ...
    function update_writer_personal_info() {
        $user_id = $this->session->userdata('userid');
    ...
    function export($what) {
        ...
        if ($what == "all") {
        ...
        } else if ($what == "penulis") {
            $data = $this->User_m->get_user_by_level('penulis');
            $header = array(
            ...
            $this->excel->setActiveSheetIndex(0);
            //name the worksheet
            $this->excel->getActiveSheet()->setTitle('Semua penulis');
    ...
    function wizard_add_writer_personal_info() {
        // 1. validate form
        $this->load->library('form_validation');
    ...
    function insert_subscriber() {
        ...
        $insert = $this->db_lib->insert_data('subscriber_pre_launch', $data);
