...
class Auth extends MY_Controller {
    ...
    function login() {
        ...
        $email = $this->input->post('login-form-email');
        $password = $this->input->post('login-form-password');

        $this->form_validation->set_rules('login-form-email', 'Email', 'required|valid_email');
        $this->form_validation->set_rules('login-form-password', 'Password', 'required|min_length[6]');
        ...
        if ($this->form_validation->run()) {
            ...
            $response['status'] = '204';
            $response['form_login_error'] = validation_errors();
        }
